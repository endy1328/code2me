...
- [ ] 새 어댑터 추가 전 합격 기준을 문서로 연결

### 2. Regression Matrix

- [x] 현재 샘플/테스트 기준 회귀 매트릭스 문서 작성
- [ ] fixture별 `profileDetection`, `flow detail`, `inferred card visibility` 점검 표 추가
- [ ] 미검증 영역별 우선순위 확정
...
- [ ] `Start Here` / `Flow Details` 우선 노출 기준 문서화
- [ ] 온보딩/아키텍처/운영이관 시나리오별 회귀 케이스 추가

### 7. Next Profile Preparation

- [ ] 다음 프로파일 후보 1~2개 선정
- [ ] 프로파일별 입력 신호 초안 작성
- [ ] 프로파일별 최소 메타모델 차이 메모 작성
- [ ] 구현은 시작하지 않고 스켈레톤 문서만 유지
...
- 완료
  ...
  - 관련 회귀 테스트 추가
- 다음 우선순위
  - regression matrix 상세화
  - 실제 프로젝트 검증 게이트 보강
  - mapping-focused fixture
  - 시나리오별 회귀 케이스
